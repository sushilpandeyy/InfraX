          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg font-semibold transition-all ${
              activeFeature === 'query'
                ...
                : 'glass border border-blue-primary/30 text-gray-300 hover:bg-blue-primary/20'
            }`}
          >
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeFeature === 'query' && (
          <div className="space-y-6">
            <div className="glass-card p-6 rounded-xl border border-blue-primary/30">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {quickQueries.map((query, index) => (
                    <button
                      ...
                      className="text-xs px-3 py-2 rounded-lg glass border border-blue-primary/30 text-gray-300 hover:bg-blue-primary/20 transition-all text-left"
                    >
                      {query}
                    </button>
                  ))}
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 bg-transparent border border-blue-primary/30 rounded-xl px-4 py-3 text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-primary/50"
                />
                <button
                  onClick={handleNLQuery}
                  disabled={queryLoading || !nlQuery.trim()}
                  className="bg-blue-primary hover:bg-blue-dark disabled:bg-gray-600 text-white px-6 py-3 rounded-xl font-semibold transition-all disabled:cursor-not-allowed"
                >
            ...
            {queryResult && queryResult.success && (
              <div className="glass-card p-6 rounded-xl border border-green-500/30">
                ...
                {queryResult.matched_resources && queryResult.matched_resources.length > 0 ? (
                  <div className="space-y-3">
                    {queryResult.matched_resources.map((match: any, index: number) => (
                      <div key={index} className="glass p-4 rounded-lg border border-blue-primary/20">
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            <p className="text-sm font-bold text-blue-light">
                              {match.resource.full_name || match.resource.name}
                            </p>
                            <p className="text-xs text-gray-400">Type: {match.resource.type}</p>
                          </div>
                          <span className="text-xs px-2 py-1 rounded bg-green-500/20 text-green-400">
                            {(match.relevance_score * 100).toFixed(0)}% match
                          </span>
                        </div>
                        <p className="text-sm text-gray-300 mb-2">{match.match_reason}</p>
                        {match.resource.properties && (
                          <div className="mt-2 p-3 bg-gray-900/50 rounded text-xs font-mono text-gray-400">
                ...
                {queryResult.suggestions && queryResult.suggestions.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-blue-primary/20">
                    <p className="text-xs text-gray-400 mb-2">Try these queries:</p>
                    <div className="flex flex-wrap gap-2">
                      {queryResult.suggestions.map((suggestion: string, index: number) => (
                        <button
                          key={index}
                          onClick={() => setNlQuery(suggestion)}
                          className="text-xs px-3 py-1 rounded-lg glass border border-blue-primary/30 text-gray-300 hover:bg-blue-primary/20"
                        >
                          {suggestion}
                        </button>
                      ))}
          ...
          <div className="space-y-6">
            <div className="glass-card p-6 rounded-xl border border-blue-primary/30">
              ...
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-xs text-gray-400 mb-2">Forecast Months</label>
                  <input
                    type="number"
                    value={costMonths}
                    ...
                    min="1"
                    max="36"
                    className="w-full bg-transparent border border-blue-primary/30 rounded-lg px-3 py-2 text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-primary/50"
                  />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 mb-2">Growth Rate (%/month)</label>
                  <input
                    type="number"
                    value={growthRate}
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-2">Usage Pattern</label>
                  <select
                    value={usagePattern}
                    onChange={(e) => setUsagePattern(e.target.value)}
                    className="w-full bg-gray-900 border border-blue-primary/30 rounded-lg px-3 py-2 text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-primary/50"
                  >
              ...
              <button
                ...
                className="w-full bg-blue-primary hover:bg-blue-dark disabled:bg-gray-600 text-white px-6 py-3 rounded-xl font-semibold transition-all disabled:cursor-not-allowed"
              >
            ...
            {costResult && costResult.success && (
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="glass-card p-6 rounded-xl border border-green-500/30">
                    <h4 className="text-xs text-gray-400 mb-2">Base Monthly Cost</h4>
                    <p className="text-3xl font-bold text-green-400" style={{ fontFamily: 'Space Grotesk' }}>
                      ${costResult.base_monthly_cost?.toFixed(2)}
                    </p>
                  </div>
                  <div className="glass-card p-6 rounded-xl border border-purple-500/30">
                    <h4 className="text-xs text-gray-400 mb-2">Yearly Total</h4>
                    <p className="text-3xl font-bold text-purple-400" style={{ fontFamily: 'Space Grotesk' }}>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {Object.entries(costResult.cost_breakdown).map(([category, cost]: [string, any]) => (
                        <div key={category} className="glass p-3 rounded-lg">
                          ...
                          <p className="text-lg font-bold text-blue-light">${cost.toFixed(2)}/mo</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {costResult.optimization_opportunities && costResult.optimization_opportunities.length > 0 && (
                  <div className="glass-card p-6 rounded-xl border border-yellow-500/30">
                    <h4 className="text-md font-bold text-yellow-400 mb-3">💡 Optimization Opportunities</h4>
            ...
            <div className="glass-card p-6 rounded-xl border border-purple-500/30">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {costQueries.map((query, index) => (
                    <button
                      ...
                      className="text-xs px-3 py-2 rounded-lg glass border border-purple-500/30 text-gray-300 hover:bg-purple-500/20 transition-all text-left"
                    >
                      {query}
                    </button>
                  ))}
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1 bg-transparent border border-purple-500/30 rounded-xl px-4 py-3 text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50"
                />
                <button
                  onClick={handleCostQuery}
                  disabled={costQueryLoading || !costQuery.trim()}
                  className="bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 text-white px-6 py-3 rounded-xl font-semibold transition-all disabled:cursor-not-allowed"
                >
              ...
              {costQueryResult && costQueryResult.success && (
                <div className="mt-4 p-4 glass border border-purple-500/30 rounded-xl">
                  <h5 className="text-sm font-bold text-purple-400 mb-2">Answer:</h5>
